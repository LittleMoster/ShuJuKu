...
+(BOOL)deleteTableWithName:(NSString*)tableName
{
    ...
    res=[[[TableTool shared]CopySqldb]executeUpdate:deleteStr];
    }
    return res;
}
+(BOOL)rePlayTable:(NSString *)newTableName oldTableName:(NSString*)oldTableName
{
    ...
    }else
    {
        ...
        res=[[[TableTool shared]CopySqldb ]executeUpdate:tableStr];
    }
    return res;
}
-(FMDatabase *)GetSqldb
...
-(BOOL)MakeNewtableWithtableName:(NSString *)tabelName
{
    ...
    {
        result= [[[TableTool shared]CopySqldb] executeUpdate:sqlStr];
    }
    return result;
}
...
-(BOOL)MakeNewtable:(NSString *)sqlString
{
    ...
    {
     result= [[[TableTool shared]CopySqldb] executeUpdate:sqlString];
    }
    return result;
}
+(TableTool *)shared
...
//        }
    ...
    BOOL beginTrans= [[[TableTool shared]CopySqldb] beginTransaction];
    BOOL sucess=NO;
    if(beginTrans)
    {
        ...
        BOOL hasFullPath=NO;
        NSString *executeQuert=[NSString stringWithFormat:@"PRAGMA table_info(%@)",tableName];
        FMResultSet *s=[[[TableTool shared]CopySqldb ] executeQuery:executeQuert];
        while ([s next]) {
            NSString*fieldName=[s stringForColumn:@"name"];
            if([fieldName isEqualToString:rowName])
            {
        ...
        {
            NSString *executeStatements=[NSString stringWithFormat:@"ALTER TABLE '%@' ADD '%@' %@",tableName,rowName,type];
            sucess=[[[TableTool shared]CopySqldb] executeStatements:executeStatements];
            if(!sucess)
            {
                [[[TableTool shared]CopySqldb]rollback];
      
               
            }
        }
        
        sucess=[[[TableTool shared]CopySqldb]commit];
        if(!sucess)
        {
            [[[TableTool shared]CopySqldb]rollback];
...
+(BOOL)isHasRowinTable:(NSString*)tableName row:(NSString*)rowName
{
     ...
     {
         NSString *sqlString=[NSString stringWithFormat:@"PRAGMA table_info(%@)",tableName];
         FMResultSet *result=[[[TableTool shared]CopySqldb]executeQuery:sqlString];
         while ([result next]) {
...
+(FMResultSet*)extecuteQuery:(NSString*)sqlStr
{
    ...
    {
        FMResultSet *res=[[[TableTool shared]CopySqldb]executeQuery:sqlStr];
        return res;
    }
}
...
+(FMResultSet*)extecuteQuery:(NSString*)sqlStr withArgumentsInArray:(NSArray*)arr
{
    ...
    }else
    {
        ...
        if (sqlStr !=nil) {
            if (arr.count>0) {
                res =[[[TableTool shared]CopySqldb]executeQuery:sqlStr withArgumentsInArray:arr];
            }else
            {
                res=[[[TableTool shared]CopySqldb]executeQuery:sqlStr];
            }
...
+(BOOL)extecuteUpdate:(NSString*)sqlStr
{
    ...
    {
        BOOL isupdate= [[[TableTool shared]CopySqldb]executeUpdate:sqlStr];
        return isupdate;
    }
}
...
+(BOOL)extecuteUpdate:(NSString*)sqlStr param:(NSArray*)arr
{
    ...
    {
       BOOL isupdate= [[[TableTool shared]CopySqldb]executeUpdate:sqlStr withArgumentsInArray:arr];
       return isupdate;
    }
}
